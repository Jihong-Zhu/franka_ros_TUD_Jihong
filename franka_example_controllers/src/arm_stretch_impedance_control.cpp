...
void ArmStretchImpedanceController::update(const ros::Time& /*time*/,
  ...
  double stiffness_distance = 0.02;
  error[0] = min(max(error[0], -stiffness_distance), stiffness_distance);
  error[1] = min(max(error[1], -stiffness_distance), stiffness_distance);
  error[2] = min(max(error[2], -stiffness_distance), stiffness_distance);
  ...
  tau_task << jacobian.transpose() *
                  (-coordTransformMatrix * cartesian_stiffness_ * coordTransformMatrix.transpose() * error -
                  coordTransformMatrix * cartesian_damping_ * coordTransformMatrix.transpose() * (jacobian * dq));
  // tau_task << jacobian.transpose() *
